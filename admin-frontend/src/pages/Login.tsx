...
export const Login: React.FC = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setAdminAuth(true, response.data.response.data.admin);
      navigate('/');
    } catch (err: any) {
    ...
    <div className="min-h-screen flex justify-center items-center relative overflow-hidden bg-background-dark">
      ...
      >
        <div className="text-center mb-8">
          ...
          <p className="text-slate-400 text-sm">Sign in to manage the internal systems</p>
        </div>
        ...
        <div className="mt-6 text-center text-sm text-slate-400">
          Want to become an admin? <Link to="/signup" className="text-primary font-semibold hover:underline">Sign up</Link>
        </div>
      </motion.div>
    </div>
  );
